...
if ( $SMTP == true ) {
	if ( $mysql == true ) {
		...
		if ( $result == 0 ) {
			...
			$applicationid = 'VS2016'. $applicationid;
			...
			$sqlpersonal = "INSERT INTO `vedica_admission`.`users_personal_details` (`application_id`, `f_name`, `m_name`, `l_name`) VALUES ('".mysql_real_escape_string( $finalusername )."','".mysql_real_escape_string( $finalfirstname )."','".mysql_real_escape_string( $finalmiddlename )."','".mysql_real_escape_string( $finallastname )."')
						ON DUPLICATE KEY
						UPDATE
			...
			$sqlcontact = "INSERT INTO `vedica_admission`.`users_contact_details` (`application_id`, `email_id`) VALUES (
