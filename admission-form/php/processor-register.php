...
if ( $SMTP == true ) {
	if ( $mysql == true ) {
		...
		if ( $result == 0 ) {
			...
			$sqlpersonal = "INSERT INTO `vedica_admission_2017`.`users_personal_details` (`application_id`, `f_name`, `m_name`, `l_name`) VALUES ('".mysql_real_escape_string( $finalusername )."','".mysql_real_escape_string( $finalfirstname )."','".mysql_real_escape_string( $finalmiddlename )."','".mysql_real_escape_string( $finallastname )."')
						ON DUPLICATE KEY
						UPDATE
